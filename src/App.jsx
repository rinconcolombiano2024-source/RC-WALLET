...
import { walletConnectConfigured } from "./wagmi-config.js";
...
export default function App() {
  ...
  const externalConnection = useConnection();
  const externalConnect = useConnect();
  const externalConnectors = useConnectors();
  const externalDisconnect = useDisconnect();
  const externalSwitchChain = useSwitchChain();
  const {
    address: connectedExternalAddress,
    connector: externalConnector,
    isConnected: externalConnected,
  } = externalConnection;
  const externalConnectionError = externalConnect.error;
  const externalConnecting = externalConnect.isPending;
  ...
  useEffect(() => {
    if (!externalConnectionError) return;
    showStatus(externalConnectionError.message, "error");
  }, [externalConnectionError, showStatus]);

  useEffect(() => {
    if (!externalConnected || !connectedExternalAddress || !targetAddress) {
      return;
    }
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      if (selectedAsset.chainId === WORLD_CHAIN_ID) {
        ...
        if (!externalMatches || !externalConnector) {
          throw new Error(
        ...
        await externalSwitchChain.mutateAsync({
          chainId: selectedAsset.chainId,
        });
        const externalProvider = await externalConnector.getProvider({
          chainId: selectedAsset.chainId,
        });
        result = {
          route: "external",
              ...
              <div className="recovery-explanation">
                ...
                <div className="wallet-connectors">
                  {externalConnectors.map((connector) => (
                    <button
                      ...
                      onClick={() =>
                        externalConnect.mutate({ connector })
                      }
                    >
                  ...
                  {externalConnected && (
                    <button
                      ...
                      type="button"
                      onClick={() => externalDisconnect.mutate()}
                    >
                      Desconectar
                    </button>
                  )}
                ...
                {connectedExternalAddress && (
                  <p className={externalMatches ? "match" : "mismatch"}>
                    Conectada: {connectedExternalAddress}
                  </p>
                )}
